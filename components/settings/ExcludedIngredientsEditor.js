// ExcludedIngredientsEditor — body-only picker for the hard exclusion
// list (ingredients the user never wants suggested).
//
// Extracted from ExcludedIngredientsSection so the editor body (helper +
// chips + inline search) can be embedded inside an ExpandableSettingsRow
// without the section's own header (which the row already provides).
//
// Design notes (carried over from the original section):
...
//   - We deliberately do NOT reuse <SearchIngredients />: that component
//     is a controlled search BAR (not a search+pick widget). It pushes
//     fetch/results plumbing onto its parent screen (see KickoffScreen).
//     A small inline search input is the right scope for settings.
//   - Backend endpoint `/ingredients/:name` returns matching ingredients;
//     same pattern KickoffScreen uses.
...
export default function ExcludedIngredientsEditor({
  value = [],
  onChange,
  showHelper = true,
}) {
  ...
  // Resolve unknown ids → names. Re-runs whenever `value` changes (after
  // an add or redux hydration). Already-cached ids are skipped.
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const id of missing) {
        try {
          ...
          // Silent: chip falls back to id placeholder text.
        }
      }
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        if (data?.result && Array.isArray(data.ingredients)) {
          // De-dupe by name (same logic as KickoffScreen) and cap results
          // so a tap-target list doesn't blow up the layout.
    ...
    <View style={styles.wrap}>
      ...
      >
        ...
        <Pressable
          ...
          style={[styles.chip, { backgroundColor: css.palette.primary500 }]}
        >
          <FontAwesome
...
export { ExcludedIngredientsEditor };
